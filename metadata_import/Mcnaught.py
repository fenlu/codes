...
def Mcnaugh_sftp(dir,log):
    #retrieve files from marcive server  
    with pysftp.Connection('ftp.books.brodart.com/', username='', password='') as sftp:
...
def main():
    ...
    downloadDir = '/Mcnaught/download'
    processedDir = 'Mcnaught/processed'
    logDir = '/Mcnaught/logs'
    fileName = "Mcnaught_file_"+date+".csv"
    logFileName = "Mcnaught_log_"+date+".log"
 
    log = open(f"{logDir}/{logFileName}", 'w', encoding='utf-8')
    csvfile = open(f"{processedDir}/{fileName}", 'w', encoding='utf-8')

    Mcnaugh_sftp(dir=downloadDir, log=log)
